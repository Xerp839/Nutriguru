...
def check_word_in_description(row, search_words):
...
def search_food_names_without_allergy(df_filtered, predicted_food, search_words):
    ...
    with Pool(cpu_count()) as pool:
        # Partially apply the check_word_in_description function with search_words
        func = partial(check_word_in_description, search_words=search_words)
        
        # Map the function to each row of the DataFrame in parallel
        results = pool.map(func, df_filtered.iterrows())
    
    # Collect printed food names and search words
    for name in results:
        if name:
            printed_food_names.add(name)
...
def check_allergy_and_search(row, allergies_lower, search_words_lower):
    ...
    if translated_description is None or translated_description.strip() == '':
        return None  # Skip this food item if translation fails or is empty
    ...
    # Check if any search word is in the food item name
    food_name_lower = row['Name'].lower()
    if any(word in food_name_lower for word in search_words_lower):
        return row['Name']
